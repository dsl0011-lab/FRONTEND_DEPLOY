...
function CursoDetallePageEstudiante() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        setLoading(true);
        const data = await apiFetch(`/estudiante/cursos/${id}/`).catch(() => null);
        if (!alive) return;
        setAsignatura(data);
      } catch (e) {
        console.error(e);
  ...
  if (Loading || !asignatura) {
    return <ComponenteLoading />;
  }

  return (
    <section className="flex flex-col gap-6 w-full h-full">
      <article className="h-full w-full p-4 flex items-start justify-center">
        <div className="flex flex-col md:text-2xl text-sm h-full w-full min-h-[200px] justify-center items-center rounded-md relative">
          <div
            className="absolute w-full h-full inset-0 bg-cover bg-center filter opacity-50"
            style={{
              backgroundImage: `URL(${imagenesRandom()})`,
          ...
          <div className="relative z-10 flex flex-col items-center justify-center text-white text-center gap-4 p-2">
            ...
            <p>
              ...
              {asignatura.descripcion || "No hay descripción recurrente"}
            </p>
            <p>
              Profesor: {asignatura.nombre_profesor}{" "}
              {asignatura.apellidos_profesor}
            </p>
          </div>
        </div>
      </article>
